...
function DadosZeradosBanner({ diagnostico, clienteId, accent = GOLD, onSynced }) {
  ...
  // Decide o tom: se tem erro, vermelho; se só zero posts, amarelo
...
function IGProfileBadge({ clienteId }) {
  const [info, setInfo] = useState(null) // { username, profile_picture_url, instagram_url }
  ...
  if (!info?.username) {
    // Fallback: badge genérico se o /status falhou ou veio sem username
    return (
...
export default function Metricas({ session }) {
  ...
  // Conectado, sync rodou, mas tem erro persistido OU posts=0 no período → diagnóstico
    ...
    <div className="p-6 max-w-[1400px] mx-auto space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
        <div className="flex items-center gap-3">
          ...
          {overview && conectado && platform === 'instagram' && (
            <IGProfileBadge clienteId={clienteId} />
          )}
